...
//http://localhost:3000/employees/firstname/Uzma
...
//http://localhost:3000/employees/search?firstname=Uzma&lastname=Shaikh
...
app.get('/employees/test', async (req, res) => {
  try {
    const employees = employeeModel.
                        find({})
                        .where('lastname').equals('Shaikh')
                        .where('salary').gte(1000.00).lte(10000.00)
                        .where('firstname').in(['Uzma', 'moksh'])
                        .limit(10)
                        .sort('-salary')
